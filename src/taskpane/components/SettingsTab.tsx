...
export function SettingsTab({ onConnectionChange }: SettingsTabProps) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {/* Connection Section */}
      <div>
        <Label weight="semibold" style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
          OpenAI-Verbindung
        </Label>
        <div
          style={{
        ...
        {error && (
          <p style={{ fontSize: 12, color: "#d32f2f", margin: "6px 0 0" }}>{error}</p>
        )}
      </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 8 }}>
          {[
            { value: "soft", label: "Sanft", desc: "Nur eindeutige Fehler" },
            { value: "standard", label: "Standard", desc: "Fehler + Stilverbesserungen" },
            { value: "strict", label: "Streng", desc: "Alles prüfen (Abschlussarbeiten)" },
          ].map((opt) => (
            <button
              key={opt.value}
              onClick={() => setMode(opt.value as "soft" | "standard" | "strict")}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 10,
                padding: "8px 12px",
                borderRadius: 8,
                border: `2px solid ${mode === opt.value ? "#0f6cbd" : "#e0e0e0"}`,
                background: mode === opt.value ? "#e8f0fe" : "white",
                cursor: "pointer",
                textAlign: "left",
            ...
            >
              <div
                style={{
                  ...
                  height: 16,
                  borderRadius: "50%",
                  border: `2px solid ${mode === opt.value ? "#0f6cbd" : "#bbb"}`,
                  display: "flex",
                  alignItems: "center",
              ...
              >
                {mode === opt.value && (
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#0f6cbd" }} />
                )}
              </div>
              <div>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        <div style={{ flex: 1 }}>
          ...
          <Select
            value={discipline}
            onChange={(_, d) => setDiscipline(d.value)}
            size="small"
            style={{ marginTop: 4 }}
          >
        ...
        <div style={{ flex: 1 }}>
          ...
          <Select
            value={citationStyle}
            onChange={(_, d) => setCitationStyle(d.value)}
            size="small"
            style={{ marginTop: 4 }}
          >
